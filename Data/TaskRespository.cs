...
namespace Sourceworx.DMS.NativeApp.Data
{
    /// <summary>
    ...
    public class TaskRepository
    {
        ...
        public async Task<List<ProjectTask>> ListAsync()
        {
            ...
            while (await reader.ReadAsync())
            {
                ...
                // Check if DocumentPath column exists and is not null
                if (reader.FieldCount > 4 && !reader.IsDBNull(4))
                {
                    task.DocumentPath = reader.GetString(4);
                }
        ...
        public async Task<int> SaveItemAsync(ProjectTask item)
        {
            ...
            {
                saveCmd.CommandText = @"
            INSERT INTO Task (Title, IsCompleted, ProjectID, DocumentPath)
            VALUES (@title, @isCompleted, @projectId, @documentPath);
            SELECT last_insert_rowid();";
            }
            else
            {
                saveCmd.CommandText = @"
            UPDATE Task SET Title = @title, IsCompleted = @isCompleted, ProjectID = @projectId, DocumentPath = @documentPath
            WHERE ID = @id";
            ...
            saveCmd.Parameters.AddWithValue("@documentPath", item.DocumentPath ?? (object)DBNull.Value);
