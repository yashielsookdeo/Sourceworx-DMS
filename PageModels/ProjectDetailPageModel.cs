...
namespace Sourceworx.DMS.NativeApp.PageModels
{
    public partial class ProjectDetailPageModel : ObservableObject, IQueryAttributable, IProjectTaskPageModel
    {
        ...
        {
            // Save the task with any updates (including document path if it was scanned)
            await _taskRepository.SaveItemAsync(task);

            // If the task has a document, show a toast notification
            if (task.HasDocument)
            {
                await AppShell.DisplayToastAsync("Document saved for task: " + task.Title);
            }
